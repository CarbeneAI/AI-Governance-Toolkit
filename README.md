...
| **[Agent Privilege Matrix](Agent-Privilege-Matrix.md)** | Govern the agents your employees already run in Claude Code, Cursor, and Codex. One row per agent (identity, data class, allowed actions, human-in-the-loop, stop control), each column mapped to a Section 10 control, plus a ready Wazuh/Specter rule that fires the moment an agent token reaches a resource it has never touched. |
| **[Healthcare AI Audit Log Schema](Healthcare-AI-Audit-Log-Schema.md)** | Build the AI audit trail before a health-system security review, an OCR inquiry, or your board asks for it. A per-inference record that ties a model output to the PHI it touched, the accountable human, and the BAA that made it lawful, with a worked example, a six-field minimum, and a Wazuh/Specter rule for AI actions that reach a patient record unreviewed. |
